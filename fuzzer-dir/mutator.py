...
MAX_REPEAT_COUNT = 300_0 # Maximum amount of repetitions

MAX_REPEAT_LENGTH = 1000 # Maximum length of the repeating stuff

MAX_REPEAT_COUNT_LINEAR = 500000

MIN_REPEAT_COUNT_LINEAR = 20000

MAX_REPEAT_TOKEN_LENGTH = 2000 # Maximum length of the string which to repeat.

MAX_REPEAT_STRING_COUNT = 1000 # Maximum amount of repeated strings, for example if we have three of these then this will generate a string like "ababababccccccccccccccqwertyqwertyqwerty" (three repeating substrings)
...
def custom_mutator(data, max_size, seed, native_mutator):
        # print("Called custom mutator!!!!")
        # debug_log("Called custom_mutator")
        ...
        # debug_log("Length of returning stuff: "+str(len(new_data)))
...
def stringmult(string: bytes, c: int) -> bytes: # Multiplies string by c times.
        if len(string) == 0:
                return "placeholder"*random.randrange(100)
...
def generate_new() -> bytes: # Generate a new ascii string.
        ...
        for i in range(repeat_count): # Generate "repeat_count" repeating strings.
                o = generate_repeating(rnum(MAX_REPEAT_COUNT))
...
def mutate_existing(data: bytes) -> bytes:
        ...
        if True and substr:
                ...
                if not isinstance(multiplication, bytes):
                        multiplication = bytes(multiplication, 'ascii')
