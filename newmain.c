...
int split_packets(char *input, char *responses[], const char *sep)
{
    int count = 0;
    char *token = strtok(input, sep);
    while (token && count < MAX_RESPONSES) {
        responses[count++] = token;
        token = strtok(NULL, sep);
    }
    return count;
}

char *packets[MAX_RESPONSES];
int num_packets = 0;

void *ftp_listener_thread(void *arg)
{
    ...
    printf("[+] FTP listener on 127.0.0.1:8081 started\n");
    char buf[1024];
    ...
    while (1) {
        ...
        for (int i = 0; i < num_packets; i++) {
            if (strlen(packets[i]) == 0) continue;

            char response_line[1024];
            snprintf(response_line, sizeof(response_line), "%s\r\n", packets[i]);
            printf("[+] Sending: %s", response_line);
            send(client_fd, response_line, strlen(response_line), 0);

            // Optionally recv a reply
        ...
        printf("[+] Got connection on 8081\n");

        // Send a minimal FTP welcome response
...
#define SEPARATOR "ABC"

static char* hardcoded_response = "GET /ftp/ HTTP/1.1\r\nHost: 127.0.0.1:8080\r\nUser-Agent: curl/8.5.0\r\nAccept: */*\r\n\r\n";
...
static void *fuzzer_thread(void *arg)
{
    ...
    while ( __AFL_LOOP(10000)) {
        ...
        // num_packets = split_packets(buf, packets, SEPARATOR);
...
pthread_create(&ftp_thread, NULL, ftp_listener_thread, NULL);
*/

static void launch_ftp_thread(void)
{
    pthread_t t;
    pthread_attr_t attr;
    ...
    pthread_create(&t, &attr, ftp_listener_thread, NULL);
}
